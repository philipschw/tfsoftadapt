...
class TFSoftAdaptBase():
    ...
    def _softmax(self,
                 ...
                 beta: float = 1,
                 numerator_weights: tf.Tensor = None,
        ...
        if shift_by_max_value:
            exp_of_input = tf.math.exp(beta * (input_tensor - tf.reduce_max(input_tensor)))
        else:
            exp_of_input = tf.math.exp(beta * input_tensor)

        # This option will be used for the "loss-weighted" variant of SoftAdapt.
        if numerator_weights is not None:
            exp_of_input = tf.multiply(numerator_weights, exp_of_input)

        return exp_of_input / (tf.reduce_sum(exp_of_input) + self.epsilon)
